...
def validation(diffusion, unet, mapper, image_encoder, tokenizer, video_data, text_encoder, vae, cfg_scale, device, save_dir):
    ...
    z = torch.randn(video_data['video'].size(0), 4, 80, 32, 32).to(device)
    ...
    with torch.no_grad():
        ...
        exemplar_latent = x[:, :, :1, :, :].repeat(1, 1, 80, 1, 1)
    ...
    for batch_idx in range(b):
        ...
        torchvision.io.write_video(f'{save_dir}/sampled_video.mp4', video_, fps=16)
